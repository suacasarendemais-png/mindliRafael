...
import { SearchIcon, ChevronDownIcon, UsersIcon, EyeIcon, PencilIcon, TrashIcon, BookOpenIcon } from '../components/Icons';
...
const Turmas: React.FC = () => {
  ...
  const fetchTurmas = useCallback(async () => {
    ...
    setError(null);
    try {
      // 1. Fetch all courses to create a lookup map
      const cursosSnapshot = await getDocs(collection(db, 'cursos'));
      const cursosMap = new Map(cursosSnapshot.docs.map(doc => [doc.id, doc.data().name as string]));

      // 2. Fetch all turmas and map course names
      ...
      const turmasData = querySnapshot.docs.map(doc => {
        ...
        return {
          ...
          courseName: data.courseId ? cursosMap.get(data.courseId) : undefined
        }
      });
            ...
            <div className="p-6 flex-grow">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center text-gray-400">
                  ...
                  {turma.courseName ? (
                    <span className="text-cyan-400 truncate">{turma.courseName}</span>
                  ) : (
                    <span className="italic">Nenhum curso vinculado</span>
                  )}
                </div>
              </div>
            </div>
